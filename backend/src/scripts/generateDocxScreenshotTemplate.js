      ...
      children: [
          ...
          children: [
            plain(
              'Everything is matched purely by position: a "Q<number>." line marks a new question, and an ' +
                '"[A]"/"[B]"/"[C]"/"[D]" line (square brackets) marks an option. For each one, either type its ' +
                'value directly right after the marker (e.g. "[A] 1.2 J") if it\'s plain text, or paste a ' +
                'screenshot on the next line (or the same line) if it\'s an equation or diagram — never both ' +
                'unless it genuinely needs both. Whatever you type is stored exactly as typed, never re-parsed, so ' +
                'there\'s no equation/font garbling risk for typed content either. Answers, marks, chapter, etc. ' +
                'all come from the paired Excel mapping sheet, matched by the same question numbers — never from ' +
                'anything typed here.'
            ),
          ],
          spacing: { after: 200 },
        ...
        new Paragraph({
          children: [
            bold('Use "[A]", not "A)". '),
            plain(
              'Word\'s AutoCorrect silently turns a typed "A)" at the start of a line into an automatic numbered ' +
                'list the moment you press Enter — the literal "A)" text disappears entirely, and every option ' +
                'then silently collapses into the question stem. Square brackets aren\'t a pattern Word ' +
                'auto-converts, so they always stay exactly as typed.'
            ),
          ],
          spacing: { after: 200 },
        }),
